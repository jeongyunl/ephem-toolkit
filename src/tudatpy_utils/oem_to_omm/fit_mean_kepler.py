"""Mean Keplerian element fitting functions for oem_to_omm.py

This module provides functions to fit mean Keplerian orbital elements to
Orbit Ephemeris Message (OEM) state vectors using J2 secular propagation.
The fitting process uses Gauss-Newton optimization to minimize position
residuals between propagated mean elements and observed OEM states.
"""
...
def _compute_mean_kepler_residuals_from_epoch_state(
    ...
    """Compute position residuals between mean-Kepler-propagated states and OEM targets.

    This function enforces internal consistency between the epoch Cartesian
    state and the mean Keplerian elements used for propagation by recomputing
    the elements from the epoch state on every call.
    ...
    """
    # Convert Cartesian to osculating Keplerian
...
def fit_mean_kepler(
    ...
    """Fit mean Keplerian elements to an OEM arc using J2 secular propagation.
    ...
    adjusted to best-fit the remainder of the arc under a mean Kepler model
    with J2 secular perturbations.
    ...
    -------
    tuple[np.ndarray, fit_common.FitDiagnostics]
        - Fitted mean Keplerian elements at epoch (6,):
          [a, e, i, omega, RAAN, M].
    ...
    # 1 mm/s (1.0e-3 m/s) perturbation balances truncation vs. round-off error for LEO orbits.
    ...
    for iteration in range(max_iterations):
        ...
        # Convergence check: relative change in RMS below machine-precision threshold (1.0e-12)
        ...
        jacobian: np.ndarray = np.zeros((num_residuals, 3))
        for p in range(3):
        ...
        # Levenberg-Marquardt regularization prevents singular matrix
        jt_j: np.ndarray = jacobian.T @ jacobian
        jt_r: np.ndarray = jacobian.T @ residuals

        # Ensure diagonal elements are non-zero (minimum 1.0e-20) for numerical stability
        diagonal: np.ndarray = np.diag(jt_j).copy()
        diagonal[diagonal < 1.0e-20] = 1.0e-20
        # Add small damping factor (1.0e-8) to diagonal for Levenberg-Marquardt regularization
        ...
        for _ in range(10):
            ...
            if not (1.0e-8 <= e_trial < 0.9999):
                scale *= 0.5
                continue
            # Guard: semi-major axis must be above Earth's surface (6.0e6 m ≈ 6000 km)
    ...
    # Convert the best-fit Cartesian state to mean Keplerian elements
...
def compute_mean_kepler_propagation_comparison(
    ...
    """Compare mean-Kepler-propagated states with OEM states at regular intervals.

    Propagates the fitted mean Keplerian elements using J2 secular rates at the
    specified interval and finds the closest OEM state for comparison.
...
def format_mean_kepler_output(
    ...
    lines.append("Mean Keplerian elements (J2 secular fit):")
    ...
    lines.append(f"    mean anomaly:          {np.degrees(M_rad):.6f} deg")

    # Derived quantities
    mean_motion_rev_day: float = kepler.semi_major_axis_to_mean_motion(a_m)
    # Convert mean motion from rev/day to rad/s (86400.0 s/day)
    ...
    if comparison:
        lines.append("")
        lines.append(
            "  Propagation comparison (Mean J2 vs OEM) at 10-minute intervals:"
        )
        lines.append("")
        # Header
