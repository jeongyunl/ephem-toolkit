...
def _filter_metadata(
    ...
    """
    matching: list[DepVarColumnMeta] = []
...
def _extract_vector(
    ...
    matches.sort(key=lambda item: cast(int, item.vector_component_index))
...
def _extract_latitude_longitude(
    ...
    latitude_dep_types: list[str] = ["latitude", "geodetic_latitude"]
    # Known dep_type names that represent longitude
    longitude_dep_types: list[str] = [
        "longitude",
        "relative_body_aerodynamic_orientation_angle",
    ]

    fallback_constraints: list[tuple[str | None, str | None]] = [
...
def _extract_relative_position(
    ...
    KeyError
        If no matching position columns are found.
    """
    candidate_dep_types: list[str] = ["relative_position"]
    fallback_constraints: list[tuple[str | None, str | None]] = [
        (satellite_name, "Earth"),
        (satellite_name, None),
        (None, "Earth"),
...
def plot_dependent_variables_from_csv(
    ...
    trajectory_animation: list[FuncAnimation | None] = []
