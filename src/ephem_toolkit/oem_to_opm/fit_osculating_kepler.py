...
def fit_osculating_kepler(
    ...
    tuple[np.ndarray, fit_common.FitDiagnostics]
        ...
        ------
        ValueError
                If fewer than two state vectors remain in the requested fit span.
    """
    ...
    finite_difference_steps_m_s: np.ndarray = np.array([1.0e-3, 1.0e-3, 1.0e-3])
...
def compute_kepler_propagation_comparison(
    ...
    for elapsed_s in comparison_times_s:
        ...
        for timestamp, state_vector in filtered_states:
            diff: float = abs(timestamp - target_timestamp)
            if diff < closest_diff:
                closest_diff = diff
                closest_state = (timestamp, state_vector)
        ...
        # Compute differences
        position_difference_m: np.ndarray = oem_state[:3] - predicted_state[:3]
        velocity_difference_m_s: np.ndarray = oem_state[3:6] - predicted_state[3:6]

        position_error_m: float = float(np.linalg.norm(position_difference_m))
        velocity_error_m_s: float = float(np.linalg.norm(velocity_difference_m_s))

        results.append(
            fit_common.PropagationComparison(
                elapsed_s=actual_elapsed_s,
                elapsed_min=actual_elapsed_s / 60.0,
                pos_err_km=position_error_m / 1000.0,
                vel_err_m_s=velocity_error_m_s,
                dx_km=position_difference_m[0] / 1000.0,
                dy_km=position_difference_m[1] / 1000.0,
                dz_km=position_difference_m[2] / 1000.0,
                dvx_m_s=velocity_difference_m_s[0],
                dvy_m_s=velocity_difference_m_s[1],
                dvz_m_s=velocity_difference_m_s[2],
            )
        )
    ...
    *,
    ...
    if comparison:
        ...
        for comparison_record in comparison:
            lines.append(
                f"    {comparison_record.elapsed_min:8.1f}  "
                f"{comparison_record.pos_err_km:10.6f}  "
                f"{comparison_record.vel_err_m_s / 1000.0:12.9f}"
            )

        # Summary statistics
        position_errors_km: list[float] = [
            comparison_record.pos_err_km for comparison_record in comparison
        ]
        velocity_errors_km_s: list[float] = [
            comparison_record.vel_err_m_s / 1000.0 for comparison_record in comparison
        ]
        lines.append("")
        lines.append("  Summary:")
        lines.append(
            f"    Position |Δr|:  min = {min(position_errors_km):.6f} km   "
            f"max = {max(position_errors_km):.6f} km   "
            f"avg = {np.mean(position_errors_km):.6f} km"
        )
        lines.append(
            f"    Velocity |Δv|:  min = {min(velocity_errors_km_s):.9f} km/s   "
            f"max = {max(velocity_errors_km_s):.9f} km/s   "
            f"avg = {np.mean(velocity_errors_km_s):.9f} km/s"
        )
