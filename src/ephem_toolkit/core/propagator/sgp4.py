"""SGP4 propagator and TLE utilities.

Provides:
- :class:`Sgp4Propagator` - SGP4 propagator wrapping TudatPy's SGP4 ephemeris
- :class:`Tle` - Parsed Two-Line Element set dataclass
- TLE I/O: :func:`read_tle`, :func:`write_tle`, :func:`format_tle_strings`
- TLE creation: :func:`create_tle_from_mean_keplerian`
- Epoch conversions: :func:`tle_epoch_to_tt_s`, :func:`tle_epoch_to_datetime`,
  :func:`datetime_to_tle_epoch`, :func:`tle_epoch_to_iso8601`, :func:`iso8601_to_tle_epoch`

Requires tudatpy for :class:`Sgp4Propagator`. Import is deferred to avoid loading
tudatpy at module level.
...
def tle_epoch_to_tt_s(epoch_year: int, epoch_day: float) -> float:
    """Convert TLE epoch to Terrestrial Time (TT) seconds since J2000.
...
def tle_epoch_to_iso8601(epoch_year: int, epoch_day: float) -> str:
    ...
    Uses :func:`time_utils.datetime_to_iso8601` for consistent ISO 8601
...
def iso8601_to_tle_epoch(iso_str: str) -> tuple[int, float]:
    ...
    Uses :func:`time_utils.iso8601_to_datetime` for consistent ISO 8601
...
class Sgp4Propagator(Propagator[Tle]):
    ...
    Initial state is a :class:`Tle` object.
