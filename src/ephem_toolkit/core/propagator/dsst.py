...
"""Default drag coefficient for atmospheric drag modeling."""

DEFAULT_DRAG_AREA_M2: float = 1.0
"""Default drag area in m² for atmospheric drag modeling."""

DEFAULT_SPACECRAFT_MASS_KG: float = 100.0
"""Default spacecraft mass in kg for atmospheric drag modeling."""

DEFAULT_SRP_COEFFICIENT: float = 1.0
"""Default solar radiation pressure coefficient."""

DEFAULT_SRP_AREA_M2: float = 1.0
"""Default solar radiation pressure area in m²."""

ATMOSPHERE_SCALE_HEIGHT_M: float = 8500.0
"""Atmospheric scale height for exponential atmosphere model (m)."""

ATMOSPHERE_REFERENCE_DENSITY_KG_M3: float = 2.62e-10
"""Reference atmospheric density at 400 km altitude (kg/m³)."""

ATMOSPHERE_REFERENCE_ALTITUDE_M: float = 400e3
"""Reference altitude for atmospheric density model (m)."""

NEAR_CIRCULAR_ECCENTRICITY_THRESHOLD: float = 1e-6
"""Eccentricity threshold below which orbit is considered near-circular."""

NEAR_EQUATORIAL_INCLINATION_THRESHOLD_DEG: float = 1.0
"""Inclination threshold (degrees) below/above which orbit is near-equatorial."""

ECCENTRICITY_SINGULARITY_THRESHOLD: float = 1e-8
"""Eccentricity threshold for J3 secular rate singularity avoidance."""

MAX_ECCENTRICITY_CLIP: float = 0.9999999
"""Maximum eccentricity value for clipping to avoid singularities."""
...
class DsstPerturbations:
    ...
    """Include J2 zonal harmonic."""
    include_j3: bool = False
    """Include J3 zonal harmonic."""
    include_j4: bool = False
    """Include J4 zonal harmonic."""

    include_drag: bool = False
    """Include atmospheric drag."""
    drag_coeff: float = DEFAULT_DRAG_COEFFICIENT
    """Drag coefficient."""
    drag_area_m2: float = DEFAULT_DRAG_AREA_M2
    """Drag area in m²."""
    mass_kg: float = DEFAULT_SPACECRAFT_MASS_KG
    """Spacecraft mass in kg."""
    atmosphere_model: str = "exponential"
    """Atmosphere model: 'exponential' or 'harris-priester'."""

    include_srp: bool = False
    """Include solar radiation pressure."""
    srp_coeff: float = DEFAULT_SRP_COEFFICIENT
    """SRP coefficient."""
    srp_area_m2: float = DEFAULT_SRP_AREA_M2
    """SRP area in m²."""

    include_sun: bool = False
    """Include Sun third-body perturbations."""
    include_moon: bool = False
    """Include Moon third-body perturbations."""
    ephemeris_source: str = "analytical"
    """Ephemeris source: 'analytical' or 'spice'."""
...
def compute_dsst_j2_short_period_corrections(
    ...
    delta_a = (
        ...
        * (
            (1.0 - 3.0 * cos2_i)
            * (
                3.0 * (1.0 + e_m * cos_theta) ** 2 / (1.0 - e_m**2) ** 2
                - 1.0 / eta
                - 1.0
            )
    ...
    e_osc = np.clip(e_osc, 0.0, MAX_ECCENTRICITY_CLIP)
...
def osculating_to_dsst_mean(
    ...
    for iteration in range(max_iter):
        ...
        delta[TRUE_ANOMALY_INDEX] = (delta[TRUE_ANOMALY_INDEX] + np.pi) % (
            2.0 * np.pi
        ) - np.pi
    ...
    mean_elements[ARGUMENT_OF_PERIAPSIS_INDEX] = mean_elements[
        ARGUMENT_OF_PERIAPSIS_INDEX
    ] % (2.0 * np.pi)
    mean_elements[MEAN_ANOMALY_INDEX] = mean_elements[MEAN_ANOMALY_INDEX] % (
        2.0 * np.pi
    )
...
class DSSTPropagator(Propagator[KeplerianState]):
    ...
    def __init__(
        ...
        self._perturbations = (
            perturbations if perturbations is not None else DsstPerturbations()
        )
