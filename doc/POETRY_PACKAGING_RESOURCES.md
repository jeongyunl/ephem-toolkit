...
| --- | --- | --- |
| OEM, OMM, TLE, and CSV files under `test/data/` | Tests and examples | Keep outside the wheel as test fixtures. Tests resolve them relative to the test files. |
...
- Do not add `test/data/`, `build/`, `dist/`, or user-generated orbit files to package data.
...
The package smoke test in `test/test_packaging_resources.py` verifies that the installed `tudatpy_utils` root is accessible through `importlib.resources`. Wheel inspection must also confirm that test fixtures and build artifacts are absent.
