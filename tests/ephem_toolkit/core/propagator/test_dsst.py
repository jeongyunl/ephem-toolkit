...
def test_propagate_dsst_zero_time():
    ...
    np.testing.assert_allclose(propagated, mean_elements, rtol=1e-12)
...
def test_propagate_dsst_j2j3j4_all_enabled():
    ...
    assert propagated.shape == (6,)
    assert np.isfinite(propagated).all()
    assert propagated[0] == mean_elements[0]
    assert propagated[1] == mean_elements[1]
    assert propagated[2] == mean_elements[2]
